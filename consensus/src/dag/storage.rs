...
pub trait DAGStorage: Send + Sync {
    ...
    fn get_ordered_anchor_ids(&self) -> anyhow::Result<Vec<((u64, Round), NodeId)>>;
...
impl DAGStorage for ConsensusDB {
    ...
    fn save_ordered_anchor_id(&self, node_id: &NodeId) -> anyhow::Result<()> {
        let epoch_round = (node_id.epoch(), node_id.round());
        Ok(self.save_data::<OrderedAnchorIdSchema>(&epoch_round, node_id)?)
    }
    ...
    fn delete_ordered_anchor_ids(&self, node_ids: Vec<NodeId>) -> anyhow::Result<()> {
        let keys = node_ids
            .into_iter()
            .map(|id| (id.epoch(), id.round()))
            .collect();
        Ok(self.delete_data::<OrderedAnchorIdSchema>(keys)?)
    }
}
