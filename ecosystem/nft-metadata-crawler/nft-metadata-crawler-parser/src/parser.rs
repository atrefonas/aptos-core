...
impl Parser {
    ...
    pub async fn parse(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(raw_image_uri) = self.model.get_raw_image_uri() {
            if self.entry.force
            ...
            self.log_info("raw_image_uri is None, skipping image optimization");
        }
        ...
        if let Some(raw_animation_uri) = self.model.get_raw_animation_uri() {
            ...
            self.log_info("raw_animation_uri is None, skipping animation optimization");
        }
